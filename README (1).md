...
| `--only=<имена>` | Применить только указанные трансформации через запятую |
| `--ext=<расширения>` | Какие расширения сканировать, по умолчанию `.vue` |

### Ограничения

Инструмент на раннем этапе — всегда сначала запускай с `--dry-run`, проверяй diff и коммить рабочее состояние перед применением изменений. Динамические имена событий в `this.$emit()`, компоненты уже на `<script setup>`, и глубокая логика через `mixins` — не покрываются автоматически.
